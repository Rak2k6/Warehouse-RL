...
# Validator requires: base_url=API_BASE_URL, api_key=API_KEY (their exact words)
API_BASE_URL = os.getenv("API_BASE_URL", "https://api.openai.com/v1")
MODEL_NAME   = os.getenv("MODEL_NAME",   "gpt-4.1-mini")
API_KEY = os.getenv("API_KEY", "placeholder")
HF_TOKEN     = os.getenv("HF_TOKEN")      # kept for HF Space auth if needed
...
MAX_QUEUE    = 20    # action=MAX_QUEUE → wait/no-op, always valid
MAX_STEPS    = 200


def parse_action(content: str) -> int:
    """Return first integer in LLM reply that is a valid action, else no-op."""
    ...
    while not done and step < MAX_STEPS:
        ...
        try:
            ...
            print(
                ...
                f"rewards={','.join(f'{r:.2f}' for r in rewards)} error={e}",
                flush=True,
            )
            return
