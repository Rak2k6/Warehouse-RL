...
def _log_openenv(data: dict, action: int):
    """Prints OpenEnv-compliant logs."""
    step_idx = data.get("step", -1)
    if step_idx == 0:
        print("[START]")
    print(f"[STEP]")
    if data.get("done") or data.get("terminated") or data.get("truncated"):
        print("[END]")
...
async def predict_action(request: Request):
    ...
    try:
        ...
        if not isinstance(obs, list):
            _log_openenv(data, fallback_action)
